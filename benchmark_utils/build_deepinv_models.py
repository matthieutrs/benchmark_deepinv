...
def build_DPIR_model(physics, in_channels=3, device='cpu'):
    ...
    if physics.__class__.__name__ == 'MRI':
        prior = SeparablePnP(denoiser=DRUNet(in_channels=1,
                                    out_channels=1,
                                    pretrained="download",
                                    train=False,
                                    device=device))
...
def build_wavelet_model(physics, in_channels=3, device='cpu'):
    ...
    level = 3
    if physics.__class__.__name__ == 'MRI':
        prior = SeparablePnP(denoiser=deepinv.models.WaveletPrior(wv="db8", level=level).to(device))
    else:
...
def build_waveletdict_model(physics, in_channels=3, device='cpu'):
    ...
    level = 3
    if physics.__class__.__name__ == 'MRI':
        prior = SeparablePnP(denoiser=deepinv.models.WaveletDict(list_wv=["db1", "db2", "db3", "db4", "db5", "db6", "db7", "db8"], level=level).to(device))
    else:
